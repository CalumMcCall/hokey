...
handConstructors = map Hand
                     [StraightFlush, FullHouse, Flush, Straight, Trips, TwoPair, OnePair, HighCard]
...
hasTwoPair (v:w:x:y:z:zs)
  ...
  | otherwise = []
  where
    firstPair = hasPairInner sortedCards
...
hasStraight (x:xs)
  -- if an ace exists then a wheel is possible, so append it to the end so we can check for wheel
  -- straights.
...
hasFullHouse cards
  ...
  | otherwise = []
  where
    pair = hasPairInner (filter (`notElem` trips) sortedCards)
    trips = hasTripsInner sortedCards
